...
**The mean of total steps taken per day is `r meanSteps` and the median is `r medianSteps`**

## What is the average daily activity pattern?
```{r DailyActivity1, echo = TRUE}
...
On average across all the days, the interval `r max_interval` to `r max_interval+5` seconds, has the maximum number of steps.


## Imputing missing values

*1. Total number of rows with NAs is **`r sum(!ok)`** *
...
**The mean of total steps taken per day is `r mean_new` and the median is `r median_new`. Yes, these values differ from that of the first values. The effect of imputing seems to lower the mean and median values.**
